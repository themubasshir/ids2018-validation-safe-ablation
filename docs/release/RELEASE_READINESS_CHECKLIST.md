...
Status: **PRE-RELEASE PREPARATION COMPLETE; NOT CLEARED TO TAG OR ARCHIVE**
Date: 2026-08-23  
Branch: `manuscript-reproducibility-cleanup`

`PASS` means the final static repository evidence satisfies the check. `BLOCKED` requires a governance or rights decision before a release tag/Zenodo archive.
...
| 3 | `CITATION.cff` valid | PASS | YAML parsing and required CFF 1.2.0 field checks pass; invented publication fields and preferred citation are absent. |
...
| 17 | Repository branch clean and synchronized | PASS | Final handoff verifies local HEAD equals `origin/manuscript-reproducibility-cleanup` with no uncommitted files. |
...
Do not create a publication release tag, mint a Zenodo DOI, merge to `main`, or submit while either governance/rights blocker is unresolved. Static validation cannot waive the rounding or archive-rights blocks.
